...
data "template_file" "env_vars" {
  ...
  vars = {
    ...
    database_connection_url = "postgresql+psycopg2://${var.database_user}:${var.database_password}@${aws_db_instance.rds.address}:${var.database_port}/${var.database_name}"
...
resource "aws_security_group" "service_security_group" {
  ...
  ingress {
    ...
    to_port         = 6789
    protocol        = "tcp"
    cidr_blocks     = ["${chomp(data.http.myip.response_body)}/32", "192.92.207.50/32"]
